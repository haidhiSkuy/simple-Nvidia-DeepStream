...
class Pipeline:
    ...
    def __init__(
        ...
        pgie_config_path: str = "/workspaces/tes2/configs/pgies/dstest1_pgie_config.txt",
        tracker_config_path: str = "/workspaces/tes2/configs/tracker/dstest2_tracker_config.txt",
        enable_osd: bool = True, 
        write_osd_analytics: bool = True,
        save_crops: bool = False,
        redis: bool | dict = False, 
    ...
    def _redis(self, frames, _, l_frame_meta: List, ll_obj_meta: List[List]):
        r = redis.Redis(
                    host=self.redis['host'], port=self.redis['port'], db=self.redis['db']
                ) 
        for frame, frame_meta, l_obj_meta in zip(frames, l_frame_meta, ll_obj_meta):
            ...
            data = {"frame_id":frame_meta.frame_num, "objects":{}}

            for obj_meta in l_obj_meta:
                track_id = obj_meta.object_id
                x1, y1, x2, y2 = rect_params_to_coords(obj_meta.rect_params)

                data["objects"]["class_id"] = obj_meta.class_id 
                data["objects"]["bbox"] = {'x1':x1, 'y1':y1, 'x2':x2, 'y2':y2}
     
            r.publish(self.redis['channel'], json.dumps(data))
    
    def _draw_frame(self, frames, _, l_frame_meta: List, ll_obj_meta: List[List]):
        for frame, frame_meta, l_obj_meta in zip(frames, l_frame_meta, ll_obj_meta):
            frame_copy = np.array(frame, copy=True, order='C')
            ...
            for obj_meta in l_obj_meta:
                ...
                thickness = 2
                frame_copy = cv2.rectangle(frame_copy, (x1, y1), (x2, y2), color, thickness)

            print(frame_meta.frame_num)
            cv2.imwrite(f"/workspaces/tes2/DeepstreamPython/app/output/frames/frame{frame_meta.frame_num}.jpg", frame_copy)
    ...
    def _add_probes(self):
        ...
        if self.redis != False: 
            tiler_sinkpad.add_probe(Gst.PadProbeType.BUFFER, self._wrap_probe(self._redis))        
